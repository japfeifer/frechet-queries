% kNN Search6 - (1 + epsilon) approximation version - Eapx variable
% 
...
%
% Same as kNN Search2, except this version tests the frechet dec proc
% for the curves with the k-number of lowest LB's, with the k+1 lowest LB

tic;

kNum = 100;

h = waitbar(0, 'kNN Search6');
...
for k = 1:size(queryTraj,1)  % do kNN search for each query traj
    ...
    [numTrajInFinRes,Emin] = PruneTraj6(k,kNum,-1); % send in a -1 so we can calc Emin and have it returned
    ...
    if numCandCurve == kNum  % if the number of pruning results = k, then just return the candidate curves
        ...
        numTrajAdd = kNum - currNumK;
        numTrajDelete = numCandCurve - kNum;
        
        % Test the frechet dec proc for the curves with the k-number of 
        % lowest LB's, with the k+1 lowest LB
        candTrajIDList = sortrows(candTrajIDList,2,'ascend'); % sort LB asc
        candTrajID2List = candTrajIDList;
        kPlusOneLB = candTrajID2List(numTrajAdd+1,2); % k+1 lowest LB
        candTrajIDList = candTrajIDList(numTrajAdd+1:end,:); % delete first numTrajAdd rows (we may append them back)
        for i = 1:numTrajAdd
            currTraj = cell2mat(trajData(candTrajID2List(i,1),1));
            decProRes = FrechetDecide(currTraj,currQueryTraj,kPlusOneLB,1);
            if decProRes == 1 % traj is definitely in result set
                kNNTrajList = [kNNTrajList; candTrajID2List(i,1)];
            else % traj may not be in result set
                candTrajIDList(end+1,:) = candTrajID2List(i,:);
            end
        end
        currNumK = size(kNNTrajList,1);
        numTrajAdd = kNum - currNumK;
        ...
        if isempty(candTrajIDList) == false && numTrajAdd > 0
...
%             if numTrajAdd <= Inf % test by only doing the add traj
    ...
    if mod(k,10) == 0
        X = ['kNN Search6: ',num2str(k),'/',num2str(numQueryTraj)];
