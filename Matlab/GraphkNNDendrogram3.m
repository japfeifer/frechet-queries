...
function GraphkNNDendrogram3(nodeID,xStart,xEnd,depth,lineColor,lineStyle)

    global tmpCCT tmpCCTDepth lowClustRad s1Stat
    ...
    if depth <= tmpCCTDepth % only process up to a certain tree depth
        ...
        if tmpCCT(nodeID,8) == 1 % node was visited
            if ismember(tmpCCT(nodeID,6),s1Stat) == 1
        ...
        if tmpCCT(nodeID,5) > 1
            ...
            if tmpCCT(tmpCCT(nodeID,2),6) == tmpCCT(nodeID,6) %&& tmpCCT(tmpCCT(nodeID,2),5) > 1
                GraphkNNDendrogram3(tmpCCT(nodeID,2),xStart,xEnd - (totalLength * (1-leftSizeFactor)),depth+1,lineColor,lineStyle);
            end
            if tmpCCT(tmpCCT(nodeID,3),6) == tmpCCT(nodeID,6) %&& tmpCCT(tmpCCT(nodeID,3),5) > 1
                GraphkNNDendrogram3(tmpCCT(nodeID,3),xStart,xEnd - (totalLength * (1-rightSizeFactor)),depth+1,lineColor,lineStyle);
            end

            % next, try and go to child with different center trajID
            if tmpCCT(tmpCCT(nodeID,2),6) ~= tmpCCT(nodeID,6) %&& tmpCCT(tmpCCT(nodeID,2),5) > 1
                GraphkNNDendrogram3(tmpCCT(nodeID,2),xStart + (totalLength * leftSizeFactor),xEnd,depth+1,lineColor,lineStyle);
            end
            if tmpCCT(tmpCCT(nodeID,3),6) ~= tmpCCT(nodeID,6) %&& tmpCCT(tmpCCT(nodeID,3),5) > 1
                GraphkNNDendrogram3(tmpCCT(nodeID,3),xStart + (totalLength * leftSizeFactor),xEnd,depth+1,lineColor,lineStyle);
            end
        end
    end
end