...
function GetPrunedNodes4(cNodeID,Q,Qid,lowBnd,upBnd)
    ...
    elseif currBestUpNodeDist >= lowBnd - cRad && ...% the node may contain a traj that we want to include
        ...
        if lowBnd1 < lowBnd2
            GetPrunedNodes4(clusterNode(cNodeID,2),Q,Qid,lowBnd1,upBnd1);
            GetPrunedNodes4(clusterNode(cNodeID,3),Q,Qid,lowBnd2,upBnd2);
        else
            GetPrunedNodes4(clusterNode(cNodeID,3),Q,Qid,lowBnd2,upBnd2);
            GetPrunedNodes4(clusterNode(cNodeID,2),Q,Qid,lowBnd1,upBnd1);
        end
