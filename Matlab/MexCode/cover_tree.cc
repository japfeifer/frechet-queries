...
inline float get_radius_node_cluster( const node& top ){
    return dist_of_scale(get_scale( top.max_dist ));
}
...
{
//    assert( top_node.max_dist == top_node.cluster_radius );
  print_space(depth);
  print(top_node.p);
  if ( top_node.num_children > 0 ) {
    print_space(depth); printf("scale = %i\n",       top_node.scale);
    print_space(depth); printf("scale_dist = %f\n",  dist_of_scale( 6-top_node.scale ) );
    print_space(depth); printf("max_dist = %f\n",    top_node.max_dist);
    print_space(depth); printf("max_dist_scale = %i\n",   get_scale( top_node.max_dist ) );
    print_space(depth); printf("max_dist_scale_rad = %f\n",  get_radius_node_cluster(top_node) );
    print_space(depth); printf("num children = %i\n",top_node.num_children);
//    print_space(depth); printf("cluster_radius = %f\n",    top_node.cluster_radius);
...
void stats_leaf_depth( const node top_node, double& accumul, int& nofLeafs, int curr_depth, int& maxLeafDepth){
  if( top_node.num_children == 0 ){
      nofLeafs++;
      maxLeafDepth = max( maxLeafDepth, curr_depth );
      accumul += curr_depth;
  } else {
    for( int i = 0; i<top_node.num_children ;i++ )
        stats_leaf_depth(top_node.children[i], accumul, nofLeafs, curr_depth+1, maxLeafDepth );
  }
}
void stats_node_arity( const node top_node, double& accumul, int& nofNonLeafs, int& maxArity ){
   if( top_node.num_children != 0 ){
      nofNonLeafs++;
      maxArity = max( maxArity, top_node.num_children );
      accumul += top_node.num_children;
      for( int i = 0; i<top_node.num_children ;i++ )
        stats_node_arity(top_node.children[i], accumul, nofNonLeafs, maxArity );
  }
}
...
void stats_compactness( const node top_node, double& accumul, int& nofEdges ){
    assert( top_node.scale >= 0 );
    
   if( top_node.num_children != 0 ){

      for( int i = 0; i<top_node.num_children; i++ ){

        if( top_node.children[i].num_children != 0 ){
//            int scaleDiff = abs(top_node.children[i].scale - top_node.scale);
//            accumul += 1.0 / pow( base, scaleDiff );
//            accumul += top_node.children[i].max_dist / top_node.max_dist;
            accumul +=  get_radius_node_cluster( top_node.children[i] ) 
                      / get_radius_node_cluster( top_node );            
            nofEdges++;
            stats_compactness(top_node.children[i], accumul, nofEdges );
        } 
      }
  }
}
...
//    top.cluster_radius = -1.0;
    top.parent = NULL;
    
    if( top.num_children == 0 ){
//        top.cluster_radius = 0.0;
        
        push( leafs, &top);
        push( depths, cur_depth);
    } else {
        for( int i = 0; i<top.num_children; i++ ){
            get_leafs_n_depths( top.children[i], leafs, depths, cur_depth+1 );
...
void count_clusters_containing( const node& top, point& p , int & result){
    ...
    if( top.p == p ){
        ...
        dist = distance( top.p, p, MAXFLOAT );
    }
    
    if( top.p == p || dist <= get_radius_node_cluster( top ) ){
        result++;
        for( int i = 0; i<top.num_children ;i++ ){
            count_clusters_containing( top.children[i], p, result );    
        }
    }
}
...
//    
//    for( int l=0; l<leafs.index; ++l ){
//        node* clusters_to_root = leafs[l]->parent;
//        while( clusters_to_root != NULL ){
//            clusters_to_root->cluster_radius =
//                    max(clusters_to_root->cluster_radius,
//                        distance(leafs[l]->p, clusters_to_root->p, MAXFLOAT));
//            clusters_to_root = clusters_to_root->parent;
//        }
//            
//    }
//}
...
//    assert( top.max_dist == top.cluster_radius );
    assert( top.num_children==0 || top.max_dist >= 0.0 );
    
    for( int i=0; i< top.num_children; ++i ){
        assert( &top == top.children[i].parent );
        check_tree( top.children[i] );
    }
}