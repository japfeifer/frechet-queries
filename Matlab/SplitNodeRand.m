...
function SplitNodeRand(cNodeID,tDistList)
    ...
    % create list of traj, but do not include the prev centre traj
    randDistList = tDistList;
    TF1 = randDistList(:,1) == prevCenterTrajID; % logical indexing to flag the centre traj
    randDistList(TF1,:) = []; % delete the prev centre traj from list
    % now randomly choose a traj as the new centre from the list
    randIndex = randi(size(randDistList,1),1);
    newCenterTrajID = randDistList(randIndex,1);
    ...
    % recursively call SplitNodeRand for clusters that have two or more traj
    if sPrev > 1
        SplitNodeRand(pNodeID,pDistList);
    end
    
    if sNew > 1
        SplitNodeRand(nNodeID,nDistList);
    end
