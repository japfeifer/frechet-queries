...
% *** this version captures and stores the left hand and right hand confidence values.
% The confidence is an integer. 0 = low, 1 = high confidence. The confidence value is
% for the enitre sequence, i.e. there are not confidence values for each frame/joint.
...
for i = 1:size(fileList,1)
    ...
    if ismember(fileName,excludeNTURGBD,'rows') == false  % this seq contains good data
        ...
        currTraj = []; leftHandConf = []; rightHandConf = [];
        ...
        for f=1:framecount
            ...
            for b=1:bodycount
                ...
                bodyInfo = fscanf(fileid,'%d',6); % read 6 integers
                leftHandConf(b,1) = bodyInfo(2);
                rightHandConf(b,1) = bodyInfo(4);
        ...
        newIdx = 10;
        for j = 1:bodycount
            CompMoveData(numSequence,newIdx) = num2cell(leftHandConf(j,1));
            CompMoveData(numSequence,newIdx+1) = num2cell(rightHandConf(j,1));
            newIdx = newIdx + 2;
        end
