...
function [phase,numSubSeq,freqSize] = GetSeqDistFreqSize2(x,seq,doCheck,lowVaryPct,highVaryPct,lastFramePct)
...
%     phase = unwrap(angle(xdft)); 
    ...
    ignoreVal = 1; % ignore the first values (this variable should be set to 1 or greater)
    ...
    freqSize = floor((N-phase)/numSubSeq);
    
    if freqSize < 5 % if size of each cut is too small
        phase = 1;
        numSubSeq = 1;
        freqSize = N;
    end
