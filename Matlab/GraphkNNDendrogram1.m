...
function GraphkNNDendrogram1(nodeID,xStart,xEnd,depth,lineColor,lineStyle)
    ...
    if depth <= tmpCCTDepth % only process up to a certain tree depth
        ...
        if tmpCCT(tmpCCT(nodeID,2),6) == tmpCCT(nodeID,6) && tmpCCT(tmpCCT(nodeID,2),4) > 0
            GraphkNNDendrogram1(tmpCCT(nodeID,2),xStart,xEnd - (totalLength * (1-leftSizeFactor)),depth+1,lineColor,lineStyle);
        end
        if tmpCCT(tmpCCT(nodeID,3),6) == tmpCCT(nodeID,6) && tmpCCT(tmpCCT(nodeID,3),4) > 0
            GraphkNNDendrogram1(tmpCCT(nodeID,3),xStart,xEnd - (totalLength * (1-rightSizeFactor)),depth+1,lineColor,lineStyle);
        end

        % next, try and go to child with different center trajID
        if tmpCCT(tmpCCT(nodeID,2),6) ~= tmpCCT(nodeID,6) && tmpCCT(tmpCCT(nodeID,2),4) > 0
            GraphkNNDendrogram1(tmpCCT(nodeID,2),xStart + (totalLength * leftSizeFactor),xEnd,depth+1,lineColor,lineStyle);
        end
        if tmpCCT(tmpCCT(nodeID,3),6) ~= tmpCCT(nodeID,6) && tmpCCT(tmpCCT(nodeID,3),4) > 0
            GraphkNNDendrogram1(tmpCCT(nodeID,3),xStart + (totalLength * leftSizeFactor),xEnd,depth+1,lineColor,lineStyle);
        end
    end
end