...
for i = 1:szComp
    ...
    for j = 1:25 % do for each joint
        ...
        for k = 1:size(currJointSeq,1)
            if currJointSeq(k,1) == 0 && currJointSeq(k,2) == 0 && currJointSeq(k,3) == 0 % joint is at origin
                if k == 1 || firstFrameFlg == 1
                    ...
                    disp(['Middle, i: ',num2str(i),' j: ',num2str(j),' k: ',num2str(k)]);
                    currJointSeq(k,:) = currJointSeq(k-1,:);
                    changeFlg = 1;
                end
            elseif firstFrameFlg == 1
                disp(['Beginning, i: ',num2str(i),' j: ',num2str(j),' k: ',num2str(k)]);
                changeFlg = 1;
                firstFrameFlg = 0;
                for m = 1:k-1
        ...
        if firstFrameFlg == 1
            error('All frames for joint are at origin.');
        end
