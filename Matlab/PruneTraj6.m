...
function [numTrajInFinRes,Emin] = PruneTraj6(Qid,kNum,Emin)
    ...
    global currNodeList nextNodeList Eapx
    ...
    if Emin == -1 % (1 + epsilon) relative approximation, calc Emin
        tmpPrunedNodes = sortrows(tmpPrunedNodes,2,'ascend'); % sort asc by LB 
        Emin = Eapx * tmpPrunedNodes(kNum,2); % get the kth smallest LB, multiply by Eapx
    end
    ...
    if doneProcess == false
        ...
        TF2 = tmpPrunedNodes(kNum+1:end,2) + Emin > kthUB; % indexes where LB > kth smallest UB, from kNum+1 to end
    ...
    % get all traj where UB - Emin < kthLB, as these will be in result set
    if donePrune == false
        tmpPrunedNodes2 = sortrows(tmpPrunedNodes,2,'ascend'); % sort asc by LB
        kthLB = tmpPrunedNodes2(kNum,2); % get the kth smallest LB
        for i = kNum - 1 : -1 : 1
            if tmpPrunedNodes(i,3) - Emin < kthLB  % use tmpPrunedNodes which is sorted asc by UB
                numTrajInFinRes = i;
                break;
            end
        end
