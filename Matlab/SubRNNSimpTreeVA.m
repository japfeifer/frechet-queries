...
function SubRNNSimpTreeVA(Qid,tau,level,sIdx,eIdx,typeQ,eVal,graphFlg)
    ...
    for i = level+1:maxLevel % traverse the simplification tree one level at a time, start at level + 1
        ...
        % process candidates within range - generate inclusion maximal candidates
        ...
        % discard any remaining candidates that are within result intervals
        if size(maximalStrRes,1) > 0
            subStr = sortrows(subStr,[8,9]); % sort by start vertex, end vertex
            k = 1;
            for j = 1:size(subStr,1)
                if subStr(j,8) > maximalStrRes(k,2)
                    if k == size(maximalStrRes,1)
                        break % no more result intervals to check
                    else
                        k = k + 1; % get the next result interval
                    end
                end
                if subStr(j,8) >= maximalStrRes(k,1) && subStr(j,8) <= maximalStrRes(k,2) && ...
                   subStr(j,9) >= maximalStrRes(k,1) && subStr(j,9) <= maximalStrRes(k,2)     % candidate is within result interval
                    subStr(j,6) = 1; % mark for deletion
                end
            end
    ...
    if graphFlg == 1
        GraphSubRNN(Qid,i,3,subStr,tau,maximalStrRes);
    end
