...
function GetPrunedNodes8(cNodeID,Q,Qid,lowBnd,upBnd,largestUB)

    global clusterNode prunedNodes nodeCheckCnt trajData prevLowestUpBnd nextNodeList
    ...
    elseif largestUB >= lowBnd - cRad && ...% the node may contain a traj that we want to include
        ...
        if lowBnd1 < lowBnd2
            ...
            GetPrunedNodes8(clusterNode(cNodeID,2),Q,Qid,lowBnd1,upBnd1,largestUB);
            nodeCheckCnt = nodeCheckCnt + 1;
            GetPrunedNodes8(clusterNode(cNodeID,3),Q,Qid,lowBnd2,upBnd2,largestUB);
        else
            nodeCheckCnt = nodeCheckCnt + 1;
            GetPrunedNodes8(clusterNode(cNodeID,3),Q,Qid,lowBnd2,upBnd2,largestUB);
            nodeCheckCnt = nodeCheckCnt + 1;
