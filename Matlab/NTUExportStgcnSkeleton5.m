...
NHIP  = [1,2,3]; NSPIN = [4,5,6]; NNECK = [7,8,9]; NHEAD = [10,11,12];
NLSHO = [13,14,15]; NLELB = [16,17,18]; NLWST = [19,20,21]; NLHND = [22,23,24];
NRSHO = [25,26,27]; NRELB = [28,29,30]; NRWST = [31,32,33]; NRHND = [34,35,36];
NLHIP = [37,38,39]; NLKNE = [40,41,42]; NLANK = [43,44,45]; NLFOT = [46,47,48];
NRHIP = [49,50,51]; NRKNE = [52,53,54]; NRANK = [55,56,57]; NRFOT = [58,59,60];
NSHO  = [61,62,63]; NLTIP = [64,65,66]; NLTHM = [67,68,69]; NRTIP = [70,71,72]; NRTHM = [73,74,75];
...
for i=1:size(currSeqSet,1) % for each sequence do
    ...
    currSeq(:,76:78) = 0; % put in a joint at the end with all-zero coordinates (origin coordinate)
