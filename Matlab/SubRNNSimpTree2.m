...
function SubRNNSimpTree2(Qid,tau,level,sIdx,eIdx,typeQ,eVal,graphFlg)
    ...
    if graphFlg == 1
        GraphSubRNN2(Qid,level,1,subStr,tau);
    end
    ...
    for i = level+1:maxLevel % traverse the simplification tree one level at a time, start at level + 1
        ...
        if graphFlg == 1
            GraphSubRNN2(Qid,i,2,subStr,tau);
        end
    ...
    if graphFlg == 1
        GraphSubRNN2(Qid,i,3,subStr,tau,resRNN);
    end
