% kNN Search5 - absolute value approximation version
% 
...
%
% Same as kNN Search, except this version tests the frechet dec proc
...
for k = 1:size(queryTraj,1)  % do kNN search for each query traj
    ...
    if numCandCurve == kNum  % if the number of pruning results = k, then just return the candidate curves
        ...
        % only process if cenTrajList is not null, and more traj need to be
...
%             if numTrajAdd <= Inf % test by only doing the add traj
    ...
    if mod(k,10) == 0
        X = ['kNN Search5: ',num2str(k),'/',num2str(numQueryTraj)];
