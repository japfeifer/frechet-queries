% Test searching for elements in a set, using hamming distance.
...
% check if elements are in set vAdd using hamming dist
...
for i = 1:numVec
    ...
    for j = 1:numVec % search vectors and find closest one
        dist = VectorHammDist(vMul,vList(j,:));
        if dist < bestDist
            bestDist = dist;
            bestID = j;
        end
    ...
    onesDist = VectorHammDist(vMul,vOnes);
