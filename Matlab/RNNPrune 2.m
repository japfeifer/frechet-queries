...
function RNNPrune(cNodeID,tau,Q,Qid,prevCentreTrajID,prevLowBnd,doCnt)

    global clusterNode S1 nodeCheckCnt trajData distCalcCnt
    ...
    if prevCentreTrajID == centerTrajID % this centre Traj is same as parent centre Traj
        ...
        lowBnd = GetBestConstLB(centreTraj,Q,tau + clusterNode(cNodeID,4),1,centerTrajID,Qid);
    ...
    if clusterNode(cNodeID,5) == 1 % at leaf 
        if lowBnd <= tau % traj is a potential candidate for result set
            cntConLB = cntConLB + 1;
            upBnd = GetBestUpperBound(centreTraj,Q,1,centerTrajID,Qid,tau,doCnt);
            if upBnd <= tau
    ...
    elseif lowBnd <= tau + clusterNode(cNodeID,4) % some or all of the cluster may be in results
        ...
        if lowBnd * 1.25 + clusterNode(cNodeID,4) < tau 
            upBnd = GetBestUpperBound(centreTraj,Q,1,centerTrajID,Qid, tau - clusterNode(cNodeID,4));
        ...
        if upBnd + clusterNode(cNodeID,4) <= tau % all leafs beneath this node are in results
            ...
            RNNPrune(clusterNode(cNodeID,2),tau,Q,Qid,centerTrajID,lowBnd,doCnt);
            RNNPrune(clusterNode(cNodeID,3),tau,Q,Qid,centerTrajID,lowBnd,doCnt);
        end
    end
end