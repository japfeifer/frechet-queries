...
tmpIdx = find(mapCanonJts(:,1) == currBestFeat);
bodyJt = mapCanonJts(tmpIdx,2);
isUsedFlg = cell2mat(aggDefStr(bodyJt,1));
trainMat = [];
testMat = [];

if isUsedFlg == 0 % this joint currently does not have a DM 
    aggDefStr(bodyJt,1) = {1}; % set isUsedFlg to 1

    % find the traj in the precompMat
    for iNTT = 1:size(precompMat,1)
        tmpDef = cell2mat(precompMat(iNTT,1));
        if tmpDef == currBestFeat
            tmpTrainMat =  cell2mat(precompMat(iNTT,3));
            tmpTestMat = cell2mat(precompMat(iNTT,5));
            break
        end
    end
    aggDefStr(bodyJt,2) = mat2cell(currBestFeat,size(currBestFeat,1),size(currBestFeat,2));
    aggDefStr(bodyJt,3) = mat2cell(tmpTrainMat,size(tmpTrainMat,1),size(tmpTrainMat,2));
    aggDefStr(bodyJt,4) = mat2cell(tmpTestMat,size(tmpTestMat,1),size(tmpTestMat,2));

else % append currBestFeat to existing Jts and create new DM

    existJts = cell2mat(aggDefStr(bodyJt,2));
    thisDef = [existJts currBestFeat];
    ...
    aggDefStr(bodyJt,2) = mat2cell(thisDef,size(thisDef,1),size(thisDef,2));
    aggDefStr(bodyJt,3) = mat2cell(trainMat,size(trainMat,1),size(trainMat,2));
    aggDefStr(bodyJt,4) = mat2cell(testMat,size(testMat,1),size(testMat,2));

end

disp(['Current best aggregated joints']);
for iNTT = 1:size(aggDefStr,1)
    isUsedFlg = cell2mat(aggDefStr(iNTT,1));
    if isUsedFlg == 1
        disp(['Body Joint: ',num2str(iNTT),'  Joint Ids: ',num2str(cell2mat(aggDefStr(iNTT,2)))]);
    end
end