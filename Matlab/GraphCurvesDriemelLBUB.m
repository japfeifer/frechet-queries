...
cmLB = GetBestConstLBLessBnd(P,Q);
cmUB = GetBestUpperBound(P,Q);
sP = size(P,1);
sQ = size(Q,1);
% plot result
figure;
...
title(['Dataset: ',dataName,...
    ...
    '   Cont Frechet LB: ',num2str(cmLB*dispFact),...
    '   UB: ',num2str(cmUB*dispFact),...
    '   P max seg len: ',num2str(maxSegLenP*dispFact),...
    '   P'' max seg len: ',num2str(maxSegLenPSimp*dispFact)]);
